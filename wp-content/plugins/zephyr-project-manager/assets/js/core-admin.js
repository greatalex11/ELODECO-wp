...
jQuery(document).ready(function ($) {
	...
	function zpmInit() {
		...
		try {
			...
			$('#zpm_colorpicker_primary, #zpm_colorpicker_primary_dark, #zpm_colorpicker_primary_light, .zpm-color-picker').wpColorPicker();
		} catch (e) { }
			...
			if ($('body').find('.zpm_kanban_container').length !== 0) {
				...
				if (container.length == 0) {
					if (typeof response.column.name !== 'undefined') {
						...
						jQuery('body').find( ".zpm_kanban_color_picker" ).wpColorPicker();
